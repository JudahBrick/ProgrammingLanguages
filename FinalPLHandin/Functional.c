...
int main(int argc, char **argv) {
    ...
    switch (argc) {
        case 3://wc FP
            results = wc(getFileText(argv[2]));
            break;
        case 4://grep word FP
            fullDoc = filterLines(getFileText(argv[3]), argv[2]);
            printf("Results: %s\n", fullDoc);
            return 0;
        case 6://grep word FP "|" wc
            results = grepWc(getFileText(argv[3]), argv[2]);
            break;
        default:
...
struct Node * grepWc(char * file, char * searchString){
    return countWords(filterNonAlpha(findWords(convertCase(filterLines(file, searchString)))));
}

struct Node * wc(char * file){
    return countWords(filterNonAlpha(findWords(convertCase(file))));
}
